...
if (isset($_GET['act']) && $_GET['act'] === 'tambah-tenagapendidik') {
    ...
    try {
        ...
        // Insert ke tabel tenaga_pendidik
        ...
        $insert_kepegawaian = "INSERT INTO kepegawaian (id_tp, id_jabatan, id_divisi, id_roledivisi,nip, status, tgl_mulai) VALUES (?, ?, ?, ?, ?, ?,?)";
        $stmt_kepegawaian = $conn->prepare($insert_kepegawaian);
        $stmt_kepegawaian->bind_param("iiiisss", $id_tp, $id_jabatan, $id_divisi, $id_roledivisi, $nip, $status, $tgl_mulai);

        if (!$stmt_kepegawaian->execute()) {
            throw new Exception("Error executing jabatan insert: " . $stmt_kepegawaian->error);
        }

        // Commit transaksi jika semua berhasil
        $conn->commit();

        echo "Data berhasil disimpan ke tiga tabel.";
    } catch (Exception $e) {
