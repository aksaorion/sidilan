...
// Pastikan cookie session diatur dengan HttpOnly dan Secure flag
ini_set('session.cookie_httponly', 1);
ini_set('session.cookie_secure', 1);
ini_set('session.use_strict_mode', 1);

// Pastikan hanya menerima data melalui HTTPS
if (!isset($_SERVER['HTTPS']) || $_SERVER['HTTPS'] !== 'on') {
    die('Harap akses halaman ini melalui koneksi HTTPS.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT * FROM tenaga_pendidik WHERE email = '$email'";
    $result = $conn->query($query);
