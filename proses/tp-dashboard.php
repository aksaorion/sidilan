...
//////////////////////////////////////////
$query_pulang = "SELECT jam FROM presensi WHERE tipe_presensi = 'Pulang' AND tanggal = CURRENT_DATE AND id_tp = $id_tp";
$result_masuk = $conn->query($query_pulang);
$jam_checkout = '';

if ($result_masuk && $result_masuk->num_rows > 0) {
    $row = $result_masuk->fetch_assoc();
    $jam_checkout = $row['jam'];
} else {
    $jam_checkout = '--:--:--';
}
